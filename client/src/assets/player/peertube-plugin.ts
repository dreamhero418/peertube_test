...
class PeerTubePlugin extends Plugin {
  ...
  gettingTotalDownload(ele:any){
    let str = ele.getAttribute('title')
    let uploaded = str.split('Total ')[1].split(': ')[1].split(' ')[0]
    console.log(uploaded)
    if (str.split('Total ')[1].split(': ')[1].split(' ')[1]==='KB'){
      uploaded =Math.floor(uploaded/1023)
    }
  ...
  private runUserWatchVideo (options: UserWatching) {
    ...
    this.userWatchingVideoInterval = setInterval(() => {
      ...
      let tooltip = document.querySelector('.vjs-peertube-displayed')      

      if (currentTime - lastCurrentTime >= 1) {
        lastCurrentTime = currentTime

        this.notifyUserIsWatching(currentTime, options.url, options.authorizationHeader,this.gettingTotalDownload(tooltip))
  ...
  private notifyUserIsWatching (currentTime: number, url: string, authorizationHeader: string,downMb:number) {
    const body = new URLSearchParams()
    body.append('currentTime', currentTime.toString())
    body.append('downMb',downMb.toString())
